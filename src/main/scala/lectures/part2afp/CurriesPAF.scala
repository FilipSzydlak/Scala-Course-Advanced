...
object CurriesPAF extends App {
  ...
  // METHOD! => you have to pass all parameter list
  def curriedAdder(x: Int)(y: Int): Int = x + y // curried method

  // convertion into a function of type Int => Int
  val add4: Int => Int = curriedAdder(4)

  // lifting => transforming a method to function => ETA-EXPANSION
  ...
  val add7 = x => simpleAddFunction(7, x)
  ...
  */
  ...
  val number = List(Math.PI, Math.E, 1, 9.8, 1.3 - 12)
  ...
  println(number.map(curriedFormatter("%14.12f"))) // compiler does sweet eta- expansion for us
  ...
  */
  ...
  byName(method) // ok
  byName(parenMethod())
  // byName(parenMethod) Scala 3 does not compile parenMethod without ()
  // BY NAME USES ONLY A VALUE OF THE METHOD,  NOT THE METHOD ITSELF
  // byName(() => 42) not ok
  byName((() => 42) ())
  ...
  //byFunction(method) not ok!!!!!!!! deos not do ETA-expansion
